...
class Network(object):
    ...
    def getAll(self, xTest = []):
        pred = []
        if xTest == [] :
            xTest = self.xTest
        for layer in self.layer:
            pred.append(self.sess.run([layer], feed_dict={self.xPlaceholder: xTest})[0].transpose())
        pred.append(self.sess.run([self.logits], feed_dict={self.xPlaceholder: xTest})[0])
        return pred
    ...
    def acc(self, yHat, yTest = [], positive = []):
        if positive == []:
            positive = range(0,yHat.shape[0])
        trueVector = []
        if yTest == []:
            yTest = self.yTest
        else:
    ...
    def getSigmoidForIndex(self, index):
        ...
        try:
            mul = tf.matmul(layer, weight)
        except:
            shape_l = layer.get_shape().as_list()
            if (shape_l == [None]):
                mul = tf.multiply(tf.reshape(layer,[1]), weight)
            else:
                mul = tf.tensordot(layer, weight, 1)
        sigmoid = tf.nn.sigmoid(tf.math.add(mul, bias))
        return sigmoid
    ...
    def getSoftmaxForIndex(self, index):
        ...
        try:
            ...
            shape_l = layer.get_shape().as_list()
            shape_w = weight.get_shape().as_list()
            if shape_l == [None]:
                mul = [3]*self.xPlaceholder[:,0] * weight
            else:
                mul = tf.tensordot(layer, weight, 1)
        softmax = tf.nn.softmax(tf.math.add(mul, bias))
        return softmax
